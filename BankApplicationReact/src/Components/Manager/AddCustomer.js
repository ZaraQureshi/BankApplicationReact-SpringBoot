  ...
  const handleSubmit = (e) => {
    ...
    if(Object.keys(errors).length === 0) {
      ...
      fetch("http://localhost:8080/customer", {
        ...
        body: JSON.stringify({
        //   customerId: Number(formValues.phone),
          customerName: formValues.fname + " " + formValues.lname,
          customerAddress: formValues.address1,
          customerPhone: formValues.phone,
          customerEmail: formValues.email,
          customerCity:formValues.city,
          customerState:formValues.state,
          customerPin:formValues.zip,
          customerGender:formValues.gender,
          mangId:Number(loggedInUser.id)
        }),
      }).then((res) => {
        if (res.status == 200) {
          return res.json();
        } else if (res.status == 500) {
          toast.error("You have entered incorrect Cutomer ID or password", {
